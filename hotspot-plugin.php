...
function render_hotspot_meta_box($post) {
    ...
    <div id="hotspots-container">
        ...
        <!-- <div id="hotspot-interface" style="width: 460px; height: auto;"> -->
        <div id="hotspot-interface">
            <img id="hotspots-image" src="<?php echo esc_url($hotspots_image_url); ?>" />
        </div>
    </div>
    ...
    <div id="hotspot-controls-container">
        <div id="hotspot-controls">
            <div id="product-selector">
                <!-- <button type="button" id="choose-product-button" class="button">製品を選択する</button> -->
                <label for="item-select">製品を選択する:</label>
  <select id="item-select">
    <?php
    // Fetch items from the custom post type 'items'
    $items = get_posts(array('post_type' => 'product', 'numberposts' => -1));
    print_r($items);
    foreach ($items as $item) {
      $image_id = get_post_thumbnail_id($item->ID);
      $image_url = wp_get_attachment_image_url($image_id, 'full');
      echo "<option value='{$item->ID}' data-image-url='{$image_url}' data-title='{$item->post_title}'>{$item->post_title}</option>";
    }
    ?>
  </select>
            ...
            <div id="hotspot-positions">
                ...
                <button type="button" id="add-hotspot" class="button button-primary">ホットスポットを追加</button>
                <p>※　製品を選択していないとホットスポットの追加ができません。</p>
                <button type="button" id="delete-mode" class="button button-secondary">削除ボタンに変更</button>
                <button type="button" id="clear-all-data" class="button button-secondary">すべてのホットスポットを削除</button>
                <div id="hotspot-json-display" style="border: 1px solid #ccc; padding: 10px; margin-top: 10px; white-space: pre-wrap;"></div>
                <button id="copy-json-button">クリップボードへコピー</button>
            </div>
        </div>
    </div>

    <button id="create-other-hotspots" class="button">他のホットスポットを追加</button>
